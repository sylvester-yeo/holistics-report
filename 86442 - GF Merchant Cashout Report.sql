...
left join xtramile.mlm_bank_details on mlm_bank_details.store_grab_id = mlm_stores.grab_id
left join food_data_service.merchant_contracts on cast(mlm_stores.grab_id as varchar) = cast(merchant_contracts.grabpay_grabid as varchar)
left join datamart.dim_merchants on merchant_contracts.merchant_id = dim_merchants.merchant_id
left join xtramile.mlm_banks on mlm_banks.id = mlm_bank_details.bank_name_id
where
--currency in ('SGD','MYR')
...
and ([[dim_merchants.merchant_id in ({{zeus_merchant_info}})]] or [[dim_merchants.merchant_name in ({{zeus_merchant_info}})]] or [[dim_merchants.business_name in ({{zeus_merchant_info}})]])
